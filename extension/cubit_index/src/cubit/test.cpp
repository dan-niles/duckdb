...
bool Cubit::init_sequential_test()
{
    ...
    if (config->encoding == EE) 
    {
        ...
        // get_val()
        old_val = get_value_rcu(/*rowid*/ ROW_ID, ReadOnce(g_timestamp), last_rub);
        assert(last_rub.type == TYPE_INV);
        assert(old_val != -1);
        assert(old_val == get_value_rcu(/*rowid*/ ROW_ID, ReadOnce(g_timestamp), last_rub));
        ...
        assert(get_value_rcu(/*rowid*/ ROW_ID, ReadOnce(g_timestamp)-1, last_rub) == old_val);
        assert(last_rub.type == TYPE_INV);
        assert(get_value_rcu(/*rowid*/ ROW_ID, ReadOnce(g_timestamp), last_rub) == to_val);
        assert(last_rub.type != TYPE_INV);
        ...
        assert(ReadOnce(g_timestamp) == 3);

        cnt2 = evaluate(0, to_val);
        std::this_thread::sleep_for(1s);
        assert(ReadOnce(g_timestamp) == 4);
        ...
        // MVCC
        assert(get_value_rcu(/*rowid*/ ROW_ID, ReadOnce(g_timestamp)-3, last_rub) == old_val);
        assert(last_rub.type != TYPE_INV);
        assert(get_value_rcu(/*rowid*/ ROW_ID, ReadOnce(g_timestamp)-2, last_rub) == to_val);
        assert(last_rub.type != TYPE_INV);
        assert(get_value_rcu(/*rowid*/ ROW_ID, ReadOnce(g_timestamp)-1, last_rub) == to_val);
        assert(last_rub.type != TYPE_INV);
        assert(get_value_rcu(/*rowid*/ ROW_ID, ReadOnce(g_timestamp), last_rub) == to_val);
        assert(last_rub.type != TYPE_INV);
        ...
        assert(get_value_rcu(l_nor, ReadOnce(g_timestamp)-2, last_rub) == -1);
        assert(get_value_rcu(l_nor, ReadOnce(g_timestamp)-1, last_rub) == to_val);
        assert(get_value_rcu(l_nor, ReadOnce(g_timestamp), last_rub) == to_val);

        // delete()
        merge_threshold = 4;
        assert(get_value_rcu(ROW_ID, ReadOnce(g_timestamp), last_rub) == to_val);
        remove(0, ROW_ID);
        assert(ReadOnce(g_timestamp) == 7);

        // Before merge
        // assert(bitmaps[to_val]->btv->getBit(ROW_ID, config) == 1);
        assert(get_value_rcu(ROW_ID, ReadOnce(g_timestamp), last_rub) == -1);
        // merge() after delete()
        merge_threshold = 1;
        cnt2 = evaluate(0, to_val);
        std::this_thread::sleep_for(1s);
        assert(ReadOnce(g_timestamp) == 8);
        if (config->segmented_btv) {
            assert(bitmaps[to_val]->seg_btv->getBit(ROW_ID, config) == 0);
        }
        else {
        ...
        assert(get_value_rcu(ROW_ID, ReadOnce(g_timestamp)-2, last_rub) == to_val);
        assert(get_value_rcu(ROW_ID, ReadOnce(g_timestamp)-1, last_rub) == -1);
        assert(get_value_rcu(ROW_ID, ReadOnce(g_timestamp), last_rub) == -1);
    }
    else if (config->encoding == RE || config->encoding == AE) 
    {
        ...
        // MVCC
        assert(get_value_rcu(/*rowid*/ ROW_ID, ReadOnce(g_timestamp)-2, last_rub) == old_val);
        assert(last_rub.type != TYPE_INV);
        assert(get_value_rcu(/*rowid*/ ROW_ID, ReadOnce(g_timestamp)-1, last_rub) == to_val);
        assert(last_rub.type != TYPE_INV);
        assert(get_value_rcu(/*rowid*/ ROW_ID, ReadOnce(g_timestamp), last_rub) == to_val);
