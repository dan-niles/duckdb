...
TransDesc * Cubit::trans_begin(int tid, uint64_t db_timestamp_t)
{
    ...
    if (!db_control) {
        ...
        while (trans->l_start_ts > ReadOnce(trans->l_end_trans->l_commit_ts)) {
            assert(ReadOnce(trans->l_end_trans->next));
    ...
    if (trans->l_start_ts < ReadOnce(trans->l_end_trans->l_commit_ts)) {
        // New trans have completed
        trans->l_start_ts = ReadOnce(trans->l_end_trans->l_commit_ts);
    }
                        ...
                        TransDesc *trans, uint32_t val, map<uint64_t, RUB> &rubs)
{
    ...
    while (__atomic_load_n(&trans, MM_ACQUIRE) && (ReadOnce(trans->l_commit_ts) <= tsp_end)) {
        if (ReadOnce(trans->l_commit_ts) > tsp_begin) 
        {   // The normal case
        ...
        } else if (ReadOnce(trans->l_commit_ts) == tsp_begin) {
    ...
    while (__atomic_load_n(&trans, MM_ACQUIRE) && (ReadOnce(trans->l_commit_ts) <= tsp_end)) {
        if (ReadOnce(trans->l_commit_ts) > tsp_begin) 
        {
            for (const auto & [row_id_t, rub_t] : trans->rubs) 
            {
                if (row_id == row_id_t) {
                    rub = rub_t;
                    rub_tsp = ReadOnce(trans->l_commit_ts);
...
int Cubit::check_conflicts(TransDesc *trans, TransDesc *tail)
{
    ...
    while (end_trans_t_2 && ReadOnce(end_trans_t_2->l_commit_ts) <= ReadOnce(tail->l_commit_ts)) 
    {
...
int Cubit::evaluate_Curve(int tid, uint32_t val)
{
    ...
    uint64_t tsp_begin_curve = ReadOnce(bitmap_old_curve->l_commit_ts);
    uint64_t tsp_end_curve = ReadOnce(trans->l_start_ts);
    while (tsp_begin_curve > tsp_end_curve) {
        #if defined(VERIFY_RESULTS)
        assert(ReadOnce(bitmap_old_curve->next));
        ...
        tsp_begin_curve = ReadOnce(bitmap_old_curve->l_commit_ts); 
        reuse_bitmap_curve = false;
    }

    map<uint64_t, RUB> rubs_curve{};
    if (tsp_begin_curve < tsp_end_curve) {
        get_rubs_on_btv(tsp_begin_curve, tsp_end_curve, ReadOnce(bitmap_old_curve->l_start_trans), val_curve, rubs_curve);
    }
    ...
    /* Need to check RUBs in between (tsp_begin, tsp_end]. */
    uint64_t tsp_begin_anchor = ReadOnce(bitmap_old_anchor->l_commit_ts);
    uint64_t tsp_end_anchor = ReadOnce(trans->l_start_ts);
    while (tsp_begin_anchor > tsp_end_anchor) {
        #if defined(VERIFY_RESULTS)
        assert(ReadOnce(bitmap_old_anchor->next));
        ...
        tsp_begin_anchor = ReadOnce(bitmap_old_anchor->l_commit_ts);
    }

    map<uint64_t, RUB> rubs_anchor{};
    if (tsp_begin_anchor < tsp_end_anchor) {
        get_rubs_on_btv(tsp_begin_anchor, tsp_end_anchor, ReadOnce(bitmap_old_anchor->l_start_trans), val_anchor, rubs_anchor);
    }
...
{   
    ...
    /* Need to check RUBs in between (tsp_begin, tsp_end]. */
    uint64_t tsp_begin = ReadOnce(bitmap_old->l_commit_ts);
    uint64_t tsp_end = ReadOnce(trans->l_start_ts);
    while (tsp_begin > tsp_end) {
        #if defined(VERIFY_RESULTS)
        assert(ReadOnce(bitmap_old->next));
        ...
        tsp_begin = ReadOnce(bitmap_old->l_commit_ts); 
        reuse_bitmap = false;
    }

    map<uint64_t, RUB> rubs{};
    if (tsp_begin < tsp_end) {
        get_rubs_on_btv(tsp_begin, tsp_end, ReadOnce(bitmap_old->l_start_trans), val, rubs);
    }
    ...
    uint64_t cnt = 0UL;
        ...
        uint64_t cnt_test = 0UL;
    #endif
    ibis::bitvector *old_btv = ReadOnce(bitmap_old->btv);
    SegBtv *old_seg_btv = ReadOnce(bitmap_old->seg_btv);
        ...
        bool *flag, int *result, RUB *rub, uint64_t *rub_tsp)
{
    ...
    for(int i = 0; i < range; i++) {
        ...
        uint64_t tsp_begin = ReadOnce(bitmap->l_commit_ts);
        ...
        while (tsp_begin > tsp_end) {
            ...
            cout << "MVCC _get_value() moves from ts: " << bitmap->l_commit_ts << 
               ...
               "   tsp_end: " << tsp_end <<
               "   g_timestamp: " << ReadOnce(g_timestamp) << endl;
            #endif
            bitmap = __atomic_load_n(&bitmap->next, MM_ACQUIRE);
            assert(bitmap);
            assert(ReadOnce(bitmap->l_commit_ts) < tsp_begin);
        
            tsp_begin = ReadOnce(bitmap->l_commit_ts); 
        }
        ...
        if (tsp_begin < tsp_end) { 
            get_rub_on_row(tsp_begin, tsp_end, ReadOnce(bitmap->l_start_trans),
        ...
        if (config->encoding == AE && config->AE_curves.count(curVal)) {
            ...
            /* Need to check RUBs in between (tsp_begin, tsp_end]. */
            uint64_t tsp_begin_anchor = ReadOnce(bitmap_anchor->l_commit_ts);
            uint64_t tsp_end_anchor = ReadOnce(l_timestamp);
            while (tsp_begin_anchor > tsp_end_anchor) {
                assert(ReadOnce(bitmap_anchor->next));
                bitmap_anchor = __atomic_load_n(&bitmap_anchor->next, MM_ACQUIRE);
                tsp_begin_anchor = ReadOnce(bitmap_anchor->l_commit_ts);
            }
            ...
            rub_tsp_t = 0UL;
            if (tsp_begin_anchor < tsp_end_anchor) {
                get_rub_on_row(tsp_begin_anchor, tsp_end_anchor, ReadOnce(bitmap_anchor->l_start_trans),
                        row_id, rub_t, rub_tsp_t);
            }
...
int Cubit::range(int tid, uint32_t start, uint32_t range) {
    ...
    if (config->segmented_btv) {
        ...
        SegBtv *old_seg_btv = ReadOnce(bitmap->seg_btv);
    ...
    for (uint32_t idx = start; idx < (start + range); idx++) {
        ...
        ibis::bitvector *old_btv = ReadOnce(bitmap->btv);
        SegBtv *old_seg_btv = ReadOnce(bitmap->seg_btv);
        ...
        map<uint64_t, RUB> rubs{};
        if (tsp_begin < tsp_end) {
            get_rubs_on_btv(tsp_begin, tsp_end, ReadOnce(bitmap->l_start_trans), idx, rubs);
        }
...
//         my_btv = ReadOnce(bitmap_old->btv);
