...
template <typename T>
inline T ReadOnce(const T &x) {
    return __atomic_load_n(&x, __ATOMIC_SEQ_CST);
}

template <typename T>
inline void WriteOnce(T &x, T val) {
    __atomic_store_n(&x, val, __ATOMIC_SEQ_CST);
}

template <typename T>
inline T CmpXchg(T *ptr, T expected, T desired) {
    T actual = expected;
    __atomic_compare_exchange_n(ptr, &actual, desired, false, __ATOMIC_SEQ_CST, __ATOMIC_RELAXED);
    return actual;
}
